...
func FormatETH(num string) string {
	...
	return fmt.Sprintf("%.4f", floatNum/math.Pow10(18)) + " CAU"
}

func FormatETHFromGwei(gwei uint64) string {
	return fmt.Sprintf("%.4f", float64(gwei)/math.Pow10(9)) + " CAU"
}
...
func FormatFullETHFromGwei(gwei uint64) string {
	return fmt.Sprintf("%v CAU", uint64(float64(gwei)/math.Pow10(9)))
}
